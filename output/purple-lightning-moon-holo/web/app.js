...
void main() {
  ...
  vec3 col=art.rgb;
  // The foil reacts to viewing angle instead of painting a new lightning
  // pattern over the supplied artwork. At rest it stays restrained; the
  // rainbow highlight wakes up as the card is tilted.
  float tilt=smoothstep(.02,.38,length(uView.xy));
  float glint=.78+.22*sin(uTime*1.5+vUv.x*6.-vUv.y*3.);
  col+=film(vUv)*sweep(vUv)*strength()*(.08+.12*tilt)*glint;
...
void main() {
  ...
  vec3 col=art.rgb;
  float tilt=smoothstep(.02,.38,length(uView.xy));
  float pulse=.88+.12*sin(uTime*1.6+vUv.y*5.);
  col+=film(vUv)*sweep(vUv)*strength()*(.08+.12*tilt)*pulse;
...
async function init() {
  refreshIcons();
  const response = await fetch("./card-config.json?rev=20260914-face-clear");
...
// Layers use the artwork's configured depths.  The effect plate is genuinely
// in front of the character, so its original lightning moves independently
// when the card is tilted.
function layoutRelief() {
  const subjectDepth = Number($("depth").value);
  const effectsDepth = Number($("fx-depth").value);
  const textDepth = Number(config.layers?.text?.depth ?? effectsDepth + 0.2);
  const invScale = 1 / Number($("scale").value);
  const place = (meshes, dz) => {
    for (const mesh of meshes) {
      mesh.position.z = subjectDepth + dz;
      mesh.scale.copy(mesh.userData.baseScale).multiplyScalar(invScale);
    }
  };
  place(reliefLayers.subject, 0);
  place(reliefLayers.effects, effectsDepth - subjectDepth);
  place(reliefLayers.text, textDepth - subjectDepth);
}
...
function reset() {
  ...
  zoom = 1;
  flipped = false;
  // The page starts with a subtle idle shimmer; reset intentionally returns
  // to a stable inspection pose.
  setAuto(false);
  faceLabels();
  const p = config.parameters || {};
  const defaults = {
