...
vec2 phase(vec3 V,out float lens){float aa=clamp(1.-fwidth(vUv.x*uPitch)*1.5,0.,1.);float a=degrees(atan(V.x,max(V.z,.05)))+(vUv.x-.5)*uSweep;float p=abs(a)/uFlip;lens=fract(vUv.x*uPitch)-.5;float q=p+lens*uStripe*.5*aa;return vec2(smoothstep(.5-uSoft,.5+uSoft,q),smoothstep(1.5-uSoft,1.5+uSoft,q));}
vec2 artUV(vec3 V,float lens){return clamp(vUv+V.xy/max(abs(V.z),.35)*uDepth*.14+vec2(lens*uRefract/uPitch,0.),0.,1.);}`;
const front = common + `void main(){vec3 V=normalize(vView);float lens;vec2 m=phase(V,lens);vec2 uv=artUV(V,lens);vec3 c=mix(mix(texture2D(tA,uv).rgb,texture2D(tB,uv).rgb,m.x),texture2D(tC,uv).rgb,m.y);vec4 txt=texture2D(tText,vUv);c=mix(c,txt.rgb,txt.a*uText);gl_FragColor=vec4(c,1.);}`;
const effect = kind => common + `void main(){vec3 V=normalize(vView);float lens;vec2 m=phase(V,lens);vec2 uv=artUV(V,lens);vec3 c=vec3(0.);float a=0.;` +
  (kind === 'foil' ? `float w=.5+.5*sin((vUv.x*.848-vUv.y*.530+V.x*2.4)*6.283*.55);c=spectrum(w+.2*V.x)*1.8;a=pow(w,12.)*uFoil*.15;` :
   kind === 'lines' ? `float line=mix(mix(texture2D(tLineA,uv).r,texture2D(tLineB,uv).r,m.x),texture2D(tLineC,uv).r,m.y);c=vec3(3.,1.7,.7);a=line*uGlow*.09;` :
...
function mat(fragment, transparent=false){return new THREE.ShaderMaterial({uniforms,vertexShader:vertex,fragmentShader:fragment,transparent,depthWrite:!transparent,side:THREE.FrontSide});}
function update(){const n=Math.abs(angle);$('angle').value=angle;$('angleValue').value=Math.round(angle)+'°';$('angel').setAttribute('aria-pressed',String(n<8&&!back));$('devil').setAttribute('aria-pressed',String(n>=8&&n<25&&!back));$('third').setAttribute('aria-pressed',String(n>=25&&!back));$('auto').setAttribute('aria-pressed',String(auto));}
...
async function init(){config=await fetch('card-config.json').then(r=>r.json());document.title=(config.title||'三相光栅')+' · 收藏卡';$('card-title').textContent=config.title;$('card-description').textContent=config.description;$('angel').textContent=config.states.a.label;$('devil').textContent=config.states.b.label;$('third').textContent=config.states.c.label;$('caption-a').textContent=config.states.a.caption;$('caption-b').textContent=config.states.b.caption;$('caption-c').textContent=config.states.c.caption;$('card-edition').textContent=(config.collection||'HOLO')+' / '+(config.edition||'LENTICULAR');
 ...
 for(const [id,key] of [['foil','uFoil'],['particles','uParticles'],['glow','uGlow']]){const input=$(id);input.value=uniforms[key].value;const change=()=>{$(id+'Value').value=Math.round(+input.value*100)+'%';uniforms[key].value=+input.value;};input.oninput=change;change();}$('loading').remove();new ResizeObserver(resize).observe(stage);resize();window.__triCard={ready:true,setTilt};animate();}
